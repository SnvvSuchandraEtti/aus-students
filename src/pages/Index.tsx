...
const Index = () => {
  ...
  const filteredStudents = useMemo(() => {
    let filtered = students.filter(student => {
      const matchesSearch = !filters.searchTerm || student.rollNumber.toLowerCase().includes(filters.searchTerm.toLowerCase());
      const matchesCampus = !filters.selectedCampus || student.campus.id === filters.selectedCampus;
      const matchesDepartment = !filters.selectedDepartment || student.department.id === filters.selectedDepartment;
      const matchesYear = !filters.selectedYear || student.year === filters.selectedYear;
      const matchesCollegeType = !filters.selectedCollegeType || student.campus.type === filters.selectedCollegeType;
    ...
    const noFiltersApplied = !filters.searchTerm && !filters.selectedCampus && !filters.selectedDepartment && !filters.selectedYear && !filters.selectedCollegeType;
  ...
  return <div className="min-h-screen relative">
        ...
        <div className="relative z-10 container mx-auto px-4 py-8 sm:py-16 mobile-hero">
          ...
          duration: 0.8
            ...
            delay: 0.2
              ...
              <img src={AdityaLogo} alt="Aditya University Logo" className="w-32 h-16 sm:w-48 sm:h-24 lg:w-64 lg:h-32 mx-auto object-contain drop-shadow-2xl" />
            </motion.div>
      ...
      <div className="container mx-auto px-4 pb-16">
        ...
        <section id="search-section" className="mb-12">
          <ScrollReveal>
            <SearchAndFilters filters={filters} onFiltersChange={setFilters} totalStudents={students.length} filteredCount={filteredStudents.length} />
          </ScrollReveal>
        </section>
        ...
        <ScrollReveal>
          ...
          delay: 0.4
            ...
            {filteredStudents.slice(0, 120).map((student, index) => <motion.div key={student.rollNumber} initial={{
            opacity: 0,
            y: 20
          }} animate={{
            opacity: 1,
            ...
            duration: 0.3
                ...
                <StudentCard student={student} onClick={handleStudentClick} index={index} onImageLoad={handleImageLoad} onImageError={handleImageError} />
              </motion.div>)}
          </motion.div>
        </ScrollReveal>
        ...
        {/* No Results */}
        {filteredStudents.length === 0 && <ScrollReveal>
            <motion.div initial={{
          opacity: 0,
          scale: 0.8
        }} animate={{
      ...
      {/* Student Modal */}
      <StudentModal student={selectedStudent} isOpen={isModalOpen} onClose={handleCloseModal} />
    </div>;
};
