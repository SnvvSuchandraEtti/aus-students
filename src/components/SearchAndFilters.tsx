...
export const SearchAndFilters = ({ 
  ...
  return (
    <motion.div
      ...
      className="glass-card p-6 rounded-2xl mb-8 sticky top-4 z-40"
    >
      {/* Search Bar */}
      <div className="relative mb-6">
        ...
        <input
          type="text"
          placeholder="Search by roll number (e.g., 0570, A0, 22A91A0570)"
          value={filters.searchTerm}
          onChange={(e) => updateFilter('searchTerm', e.target.value)}
          className="w-full pl-12 pr-4 py-4 bg-background/50 border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all duration-300 text-foreground placeholder:text-muted-foreground"
        />
        {filters.searchTerm && (
          <motion.button
            initial={{ scale: 0 }}
