...
export const SearchAndFilters = ({ filters, onFiltersChange }: SearchAndFiltersProps) => {
  ...
  const activeChips = useMemo(() => {
    const chips: { key: keyof SearchFilters; label: string; clearValue: string | boolean; colorClass: string }[] = [];
    if (filters.selectedCollegeType) {
      const ct = COLLEGE_TYPES.find(t => t.id === filters.selectedCollegeType);
      if (ct) chips.push({ key: 'selectedCollegeType', label: `${ct.icon} ${ct.name}`, clearValue: '', colorClass: 'text-cyan-500 bg-cyan-500/10 hover:bg-cyan-500/20' });
    }
    if (filters.isLateralEntry) chips.push({ key: 'isLateralEntry', label: '🔄 Lateral Entry', clearValue: false, colorClass: 'text-orange-500 bg-orange-500/10 hover:bg-orange-500/20' });
    if (filters.selectedCampus) {
      const c = CAMPUSES.find(c => c.id === filters.selectedCampus);
      if (c) chips.push({ key: 'selectedCampus', label: c.name, clearValue: '', colorClass: 'text-purple-500 bg-purple-500/10 hover:bg-purple-500/20' });
    }
    if (filters.selectedDepartment) {
      const d = DEPARTMENTS.find(d => d.id === filters.selectedDepartment);
      if (d) chips.push({ key: 'selectedDepartment', label: `${d.icon} ${d.name}`, clearValue: '', colorClass: 'text-green-500 bg-green-500/10 hover:bg-green-500/20' });
    }
    if (filters.selectedYear) {
      const y = yearOptions.find(o => o.value === filters.selectedYear);
      if (y) chips.push({ key: 'selectedYear', label: y.label, clearValue: '', colorClass: 'text-orange-500 bg-orange-500/10 hover:bg-orange-500/20' });
    }
    return chips;
  }, [filters, yearOptions]);

  return (
    <div className="relative rounded-2xl bg-gradient-to-br from-card/95 to-muted/80 backdrop-blur-xl border border-[rgba(0,0,0,0.08)] dark:border-[rgba(255,255,255,0.08)] shadow-[0_4px_20px_rgba(0,0,0,0.05)] dark:shadow-[0_4px_20px_rgba(0,0,0,0.2)] sticky top-3 z-40">
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold text-foreground uppercase tracking-wider flex items-center gap-1.5">
              Filters <span className="w-1.5 h-1.5 rounded-full bg-primary inline-block"></span>
            </span>
            <AnimatePresence>
              {activeFilterCount > 0 && (
          ...
          {filters.selectedCollegeType === 'engineering' && (
            ...
            >
              <div className="flex items-center gap-2.5 px-3 py-2 rounded-xl bg-muted/30 border border-border/40 w-fit">
                ...
                <span className="text-[11px] font-bold text-green-500">Regular</span>
                <Switch
                  checked={filters.isLateralEntry}
                  onCheckedChange={(checked) => updateFilter('isLateralEntry', checked)}
                />
                <span className="text-[11px] font-bold text-orange-500">Lateral Entry</span>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
            ...
            >
              {activeChips.map(chip => (
                <motion.button
                  ...
                  className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-medium transition-colors ${chip.colorClass}`}
                >
                  {chip.label}
